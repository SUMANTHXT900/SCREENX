/**
 * Session-pointer handoff (plan: storage/handoff/sessionPointer.ts).
 * Single source; src/storage/captureHandoff.ts re-exports for compat.
 */
...
export async function storePendingCapture(capture: PendingCapture): Promise<void> {
  ...
  try {
    await putCapture({
      ...
      partIndex: capture.partIndex,
      partTotal: capture.partTotal,
    });
  } catch (e) {
