/**
 * Tab capture client — Chrome API wrapper with rate-limiting & retries
 * (plan: capture/client/tabCaptureClient.ts).
 * Single source; src/capture/captureVisibleTab.ts re-exports for compat.
 */
    ...
    const result = await new Promise<string>((resolve, reject) => {
      ...
      try {
        if (typeof windowId === "number") {
          ...
          (chrome.tabs.captureVisibleTab as unknown as (opts: unknown, cb: (d: string) => void) => void)(options, cb);
        }
      } catch (e) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (/Rate limited/i.test(err.message) && attempt < maxRetries) {
        console.warn(
          `[ScreenX] captureVisibleTab rate limited, retry ${attempt + 1}/${maxRetries} after ${MIN_INTERVAL_MS}ms`,
          JSON.stringify({ attempt, message: err.message })
        );
